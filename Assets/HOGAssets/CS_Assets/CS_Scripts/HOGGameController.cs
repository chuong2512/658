			...
			if ( edgeLeft && edgeTop && edgeRight && edgeBottom )
			{
				...
				while ( areaHeightCurrent <= areaHeight )
				{
					...
					while ( areaWidthCurrent <= areaWidth )
					{
						...
						newObject = Instantiate( objectList[randomIndex], new Vector3(edgeLeft.position.x + areaWidthCurrent, edgeBottom.position.y + areaHeightCurrent,0), Quaternion.identity) as Transform;
				...
				while ( index < numberOfHiddenObjects )
				{
					...
					newObject = Instantiate( objectList[hiddenObjectIndex], new Vector3( Random.Range(edgeLeft.position.x, edgeRight.position.x), Random.Range(edgeBottom.position.y, edgeTop.position.y),0), Quaternion.identity) as Transform;
